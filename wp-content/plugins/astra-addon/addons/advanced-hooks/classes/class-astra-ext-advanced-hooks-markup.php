...
<div class="field__item"><strong>' . $product->get_sku() . '</strong></div>
            ...
            if (!is_single()) {
                ...
                foreach ($formatted_attributes as $key => $value) {
                    if ($count < 5) {
                        $content .= '<div>
<div class="field-wcw field field--name-attribute-product-family field--type-entity-reference field--label-inline">
<div class="field__label"><strong>' . $key . ': </strong><span style="font-size: 16px;">' . $value . '</span></div>
</div>
</div>';
                    }
                    $count++;
            ...
            foreach ($result as $post_id => $post_data) {
                ...
                if (ASTRA_ADVANCED_HOOKS_POST_TYPE != $post_type) {
                    ...
                    if (isset($layout[0]) && '404-page' == $layout[0] && 0 == $layout_404_counter) {
                        ...
                        add_filter(
                                'render_block', function ( $content, $parsed_block ) use ( $post_id ) {
